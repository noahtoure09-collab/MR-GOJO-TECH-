...
export async function linkDetection(client, message) {
    ...
    try {
        ...
        if (setting.action === 'warn') {
            ...
            await client.sendMessage(groupId, {
                text: `🚫 *ARRÊTE d'envoyer t'es lien ici connard*\n@${senderId.split('@')[0]} - ${platforms.join('/')}`,
                mentions: [senderId]
            })
        }
...
export async function kickall(client, message) {
    ...
    try {
        ...
        await client.sendMessage(groupId, { text: '𝓜𝓡 𝓰𝓸𝓳𝓸 ✨𝓼𝓽𝓲𝓬𝓴𝓮𝓻𝓼 𝓯𝓪𝓬𝓽𝓸𝓻𝔂💦- Purge...' })
...
export async function kickall2(client, message) {
    ...
    try {
        ...
        await client.sendMessage(groupId, { text: 'merci de m'avoir nommé Bye- One Shot...' })
